...
export function renderCosmosDocumentToScene(
  document: CosmosDocument,
  options: CosmosSceneRenderOptions = {},
): CosmosRenderScene {
...
function renderSystemScope(
  document: CosmosDocument,
  galaxy: CosmosGalaxy | null,
  system: CosmosSystem | null,
  width: number,
  height: number,
  padding: number,
  zoom: number,
...
function renderGalaxyScope(
  ...
  const nodes: CosmosSceneNode[] = [];
  const links: CosmosSceneLink[] = [];
  if (!targetGalaxy) {
...
function renderUniverseScope(
  ...
  padding: number,
  zoom: number,
  activeGalaxy: CosmosGalaxy | null,
  activeSystem: CosmosSystem | null,
): CosmosRenderScene {
  const nodes: CosmosSceneNode[] = [];
  const links: CosmosSceneLink[] = [];
  const centerX = width / 2;
...
function resolveGalaxy(document: CosmosDocument, activeGalaxyId?: string | null): CosmosGalaxy | null {
  if (activeGalaxyId) {
...
function createLink(fromId: string, toId: string): CosmosSceneLink {
  return {
