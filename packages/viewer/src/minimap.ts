...
export function renderViewerMinimap(
  scene: RenderScene,
  state: ViewerState,
  visibleObjects: RenderSceneObject[],
): string {
  ...
  const scale = Math.min(
    (MINIMAP_WIDTH - MINIMAP_PADDING * 2) / Math.max(bounds.width, 1),
    (MINIMAP_HEIGHT - MINIMAP_PADDING * 2) / Math.max(bounds.height, 1),
  );
  const translateX =
    (MINIMAP_WIDTH - bounds.width * scale) / 2 - bounds.minX * scale;
  const translateY =
    (MINIMAP_HEIGHT - bounds.height * scale) / 2 - bounds.minY * scale;
...
function minimapColorForObject(type: RenderSceneObject["object"]["type"]): string {
  switch (type) {
    case "star":
      return "#ffcc67";
    case "planet":
