import {
  ...
  type WorldOrbitDiagnostic,
} from "@worldorbit/core";

import type {
  CosmosContainerFields,
  CosmosDocument,
  CosmosGalaxy,
  CosmosSystem,
  CosmosUniverse,
...
export function parseCosmosDocument(source: string): CosmosDocument {
...
function parseUniverse(
  ...
  startIndex: number,
  indent: number,
): { universe: CosmosUniverse; nextIndex: number } {
...
function parseGalaxy(
  ...
  indent: number,
  universeId: string,
): { galaxy: CosmosGalaxy; nextIndex: number } {
  const header = analyzeLine(lines[startIndex], startIndex);
  if (!header) {
...
function parseSystem(
  ...
  try {
    ...
    try {
      ...
      systemBase.diagnostics.push(diagnosticFromError(error, "normalize", "cosmos.system.materialize.failed"));
    }
  } catch (error) {
    systemBase.diagnostics.push(diagnosticFromError(error, "parse", "cosmos.system.parse.failed"));
  }

  return { system: systemBase, nextIndex: index };
}

function createUniverse(id: string): CosmosUniverse {
  return {
...
function createBaseFields<TKind extends "universe" | "galaxy" | "system">(
  kind: TKind,
  id: string,
): CosmosContainerFields & { kind: TKind; id: string } {
  if (!id) {
...
function applyContainerField(
  container: CosmosContainerFields,
  tokens: string[],
  line: number,
...
function collectSystemDiagnostics(universe: CosmosUniverse): WorldOrbitDiagnostic[] {
