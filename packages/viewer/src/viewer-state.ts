...
export function clampScale(scale: number, constraints: ViewerConstraints): number {
  return Math.min(Math.max(scale, constraints.minScale), constraints.maxScale);
}

export function panViewerState(state: ViewerState, dx: number, dy: number): ViewerState {
  return {
    ...state,
...
export function rotateViewerState(state: ViewerState, deltaDeg: number): ViewerState {
  return {
    ...state,
...
export function zoomViewerStateAt(
  scene: RenderScene,
  state: ViewerState,
  factor: number,
...
export function fitViewerState(
  scene: RenderScene,
  state: ViewerState,
  constraints: ViewerConstraints,
): ViewerState {
...
export function focusViewerState(
  scene: RenderScene,
  state: ViewerState,
  objectId: string,
  constraints: ViewerConstraints,
...
export function composeViewerTransform(scene: RenderScene, state: ViewerState): string {
  const center = getSceneCenter(scene);
