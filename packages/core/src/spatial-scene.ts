...
export function renderDocumentToSpatialScene(
  document: WorldOrbitDocument,
  options: SpatialSceneRenderOptions = {},
): SpatialScene {
  ...
  const scaleModel = resolveSpatialScaleModel(
    scene.layoutPreset,
    options.spatialScaleModel,
  );
  const sceneCenter = {
  ...
  const rawMotionMetrics = new Map<string, number>();
  ...
  const minimumMotionMetric =
    Math.min(
      ...[...rawMotionMetrics.values()].filter(
        (value) => Number.isFinite(value) && value > 0,
      ),
    ) || 1;

  const positionCache = new Map<string, CoordinatePoint3D>();
  const spatialObjects = scene.objects.map((entry) =>
    createSpatialObject(
      entry,
      scene,
      sceneCenter,
  ...
  const spatialObjectMap = new Map(
    spatialObjects.map((object) => [object.objectId, object]),
  );

  const spatialOrbits = scene.orbitVisuals.map((orbit) =>
    createSpatialOrbit(
      orbit,
      spatialObjectMap,
      minimumMotionMetric,
      scene.activeEventId !== null,
    ),
  );
...
function createSpatialObject(
  entry: RenderScene["objects"][number],
  scene: RenderScene,
  sceneCenter: { x: number; y: number },
  objectMap: Map<string, RenderScene["objects"][number]>,
  orbitMap: Map<string, RenderOrbitVisual>,
  scaleModel: SpatialScaleModel,
  positionCache: Map<string, CoordinatePoint3D>,
  minimumMotionMetric: number,
): SpatialSceneObject {
  const position = resolveSpatialObjectPosition(
    entry,
    scene,
    sceneCenter,
    objectMap,
  ...
  return {
    ...
    position,
    radius: clampNumber(
      entry.radius * scaleModel.bodyRadiusMultiplier,
      scaleModel.minBodyRadius,
      scaleModel.maxBodyRadius,
    ),
    visualRadius: clampNumber(
      entry.visualRadius * scaleModel.bodyRadiusMultiplier,
      scaleModel.minBodyRadius,
      scaleModel.maxBodyRadius + 24,
    ),
    label: entry.label,
...
function resolveSpatialObjectPosition(
  entry: RenderScene["objects"][number],
  scene: RenderScene,
  sceneCenter: { x: number; y: number },
  objectMap: Map<string, RenderScene["objects"][number]>,
  orbitMap: Map<string, RenderOrbitVisual>,
  cache: Map<string, CoordinatePoint3D>,
): CoordinatePoint3D {
  ...
  let position: CoordinatePoint3D;
...
export function evaluateSpatialSceneAtTime(
  scene: SpatialScene,
  timeSeconds: number,
): Map<string, CoordinatePoint3D> {
  const objectMap = new Map(scene.objects.map((object) => [object.objectId, object]));
  const cache = new Map<string, CoordinatePoint3D>();

  const resolvePosition = (objectId: string): CoordinatePoint3D => {
    ...
    if (object.motion && object.parentId) {
      ...
      const local = computeOrbitPosition(
        object.motion,
        scene.timeFrozen ? 0 : timeSeconds,
      );
...
function computeOrbitPosition(
  motion: OrbitalMotionModel,
  timeSeconds: number,
): CoordinatePoint3D {
  const angleDeg =
    motion.phase0Deg + motion.angularVelocityDegPerSecond * timeSeconds;
...
function createMotionModel(
  ...
  orbit: RenderOrbitVisual | undefined,
  minimumMotionMetric: number,
  frozen: boolean,
): OrbitalMotionModel | null {
  ...
  const visualDurationSeconds = periodSeconds
    ...
    : clampNumber(
        FASTEST_VISIBLE_ORBIT_SECONDS * Math.pow(ratio, 0.75),
        FASTEST_VISIBLE_ORBIT_SECONDS,
        SLOWEST_VISIBLE_ORBIT_SECONDS,
      );

  return {
    phase0Deg: unitValueToDegrees(placement.phase) ?? 0,
    rotationDeg:
      unitValueToDegrees(placement.angle) ??
      orbit?.rotationDeg ??
      0,
...
function resolveRawOrbitMetric(
  placement: Extract<WorldOrbitObject["placement"], { mode: "orbit" }>,
): number {
  const distance =
    unitValueToDistanceMetric(placement.semiMajor) ??
...
function unitValueToDistanceMetric(value: UnitValue | undefined): number | null {
  if (!value) {
    return null;
  }
...
function unitValueToDurationSeconds(value: UnitValue | undefined): number | null {
  if (!value) {
    return null;
  }
...
function resolveSpatialScaleModel(
  layoutPreset: SceneLayoutPreset,
  overrides: Partial<SpatialScaleModel> | undefined,
): SpatialScaleModel {
...
function defaultSpatialScaleModel(layoutPreset: SceneLayoutPreset): SpatialScaleModel {
  switch (layoutPreset) {
    case "compact":
      return {
...
function calculateSpatialBounds(objects: SpatialSceneObject[]): SpatialBounds {
  if (objects.length === 0) {
    return {
      minX: 0,
      minY: 0,
...
function addPoint3D(
  left: CoordinatePoint3D,
  right: CoordinatePoint3D,
): CoordinatePoint3D {
  return {
    x: left.x + right.x,
    y: left.y + right.y,
