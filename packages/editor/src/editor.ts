import {
  ...
  type RenderScene,
  type WorldOrbitGalaxy,
  type WorldOrbitHierarchyDocument,
  type WorldOrbitHierarchyScope,
  type WorldOrbitHierarchySystem,
  type UnitValue,
...
interface HistoryEntry {
  ...
  hierarchyScope: WorldOrbitHierarchyScope;
...
interface HierarchySelectionDescriptor {
  kind: "universe" | "galaxy" | "system";
  id: string;
  label: string;
}
...
export function createWorldOrbitEditor(
  ...
  let hierarchyScope: WorldOrbitHierarchyScope = initial.hierarchyScope;
  ...
  const api: WorldOrbitEditor = {
    ...
    exportSvg(): string {
      if (hierarchyDocument && hierarchyScope !== "system") {
        return renderHierarchySceneToSvg(
          renderHierarchyDocumentToScene(hierarchyDocument, {
  ...
  function ensureStageRenderer(): void {
    if (hierarchyDocument && hierarchyScope !== "system") {
      if (viewer) {
        viewer.destroy();
        viewer = null;
    ...
    if (!viewer) {
      ...
      viewer = createInteractiveViewer(stage, {
        ...
        onSelectionChange(selectedObject) {
          ...
          setSelection(
            selectedObject ? { kind: "object", id: selectedObject.objectId } : { kind: "system" },
            false,
            true,
          );
        },
    ...
    viewer.setScene(
      renderDocumentToScene(materializeAtlasDocument(atlasDocument), {
        width: options.viewerWidth,
        height: options.viewerHeight,
      }),
    );
  ...
  function resolveLoadedEditorState(
    ...
    const nextAtlasDocument =
      loaded.atlasDocument ?? upgradeDocumentToV2(loaded.document);
    return {
      atlasDocument: nextAtlasDocument,
      ...
      diagnostics: mergeDiagnostics(
        resolveAtlasDiagnostics(nextAtlasDocument, loaded.diagnostics),
        collectDocumentDiagnostics(nextAtlasDocument),
      ),
    };
  }

  function collectCurrentDiagnostics(): AtlasResolvedDiagnostic[] {
    if (hierarchyDocument && hierarchyScope !== "system") {
  ...
  function commitCurrentAtlasIntoHierarchy(): void {
    ...
    for (const galaxy of hierarchyDocument.universe.galaxies) {
      ...
      hierarchyDocument.diagnostics = hierarchyDocument.universe.galaxies.flatMap((entry) =>
        entry.systems.flatMap((item) => item.diagnostics),
      );
      return;
    }
  }

  function setHierarchyScope(nextScope: WorldOrbitHierarchyScope): void {
    if (!hierarchyDocument) {
      return;
    }
  ...
  function formatHierarchySource(document: WorldOrbitHierarchyDocument): string {
    const lines: string[] = ["schema 4.0", "", `universe ${document.universe.id}`];
  ...
  function renderToolbar(): void {
    ...
    toolbar.innerHTML = `
      ...
      <div class="wo-editor-toolbar-group">
        <button type="button" data-editor-action="remove"${
          !editingSystemScope || !selection || selection.kind === "system" || selection.kind === "defaults" ? " disabled" : ""
        }>Remove</button>
  ...
  function renderInspector(): void {
    ...
    if (hierarchyDocument && hierarchyScope !== "system") {
      inspector.innerHTML = renderHierarchyInspector(
        hierarchyDocument,
        hierarchyScope,
        activeGalaxyId,
        activeSystemId,
  ...
  function handleToolbarChange(event: Event): void {
    ...
    if (target.dataset.editorScope) {
      setHierarchyScope(target.value as WorldOrbitHierarchyScope);
    }
  }

  function handleOutlineClick(event: MouseEvent): void {
    const hierarchyButton = (event.target as HTMLElement | null)?.closest<HTMLButtonElement>("[data-hierarchy-kind]");
    if (hierarchyButton && hierarchyDocument) {
      const kind = hierarchyButton.dataset.hierarchyKind as HierarchySelectionDescriptor["kind"];
...
function resolveInitialLoadedEditorState(
  loaded: LoadedWorldOrbitSource,
  source: string,
): LoadedEditorState {
  if (loaded.hierarchyDocument) {
...
function findHierarchyGalaxy(
  document: WorldOrbitHierarchyDocument,
  galaxyId: string | null | undefined,
): WorldOrbitGalaxy | null {
  if (!galaxyId) {
    return null;
  }
...
function findHierarchySystem(
  document: WorldOrbitHierarchyDocument,
  systemId: string | null | undefined,
): WorldOrbitHierarchySystem | null {
  if (!systemId) {
    return null;
  }
  for (const galaxy of document.universe.galaxies) {
...
function renderHierarchyOutlineSection(
  document: WorldOrbitHierarchyDocument,
  ...
  return `<div class="wo-editor-outline-section">
    ...
    ${document.universe.galaxies
      .map(
        (galaxy) => `${renderHierarchyOutlineButton("galaxy", galaxy.id, galaxy.title ?? galaxy.id, scope === "galaxy" && galaxy.id === activeGalaxyId)}
        ${galaxy.systems
          .map((system) =>
            renderHierarchyOutlineButton(
              "system",
              system.id,
              system.title ?? system.id,
              scope === "system" && system.id === activeSystemId,
              "wo-editor-outline-button is-compact",
            ),
          )
          .join("")}`,
      )
...
function renderHierarchyOutlineButton(
  kind: HierarchySelectionDescriptor["kind"],
  id: string,
  label: string,
  active: boolean,
  className = "wo-editor-outline-button",
): string {
  return `<button
      type="button"
...
function renderHierarchyInspector(
  document: WorldOrbitHierarchyDocument,
  scope: WorldOrbitHierarchyScope,
  activeGalaxyId: string | null,
  activeSystemId: string | null,
  ...
  if (scope === "galaxy") {
    ...
    return `<article class="wo-editor-inspector-card">
      ...
      ${galaxy.systems
        .map((system) =>
          renderHierarchyOutlineButton("system", system.id, system.title ?? system.id, system.id === activeSystemId),
        )
        .join("")}
    </article>`;
  }
...
function buildHierarchyEmbedMarkup(source: string, scope: WorldOrbitHierarchyScope): string {
  return `<div class="worldorbit-embed" data-schema="4.0" data-scope="${escapeHtml(scope)}"><pre>${escapeHtml(
    source,
  )}</pre></div>`;
}

function appendHierarchyContainerFields(
  lines: string[],
  container: {
    title: string | null;
...
function extractSystemBlockLines(system: WorldOrbitHierarchySystem): string[] {
  ...
  const blockLines = (startIndex >= 0 ? lines.slice(startIndex) : [`system ${system.id}`]).map((line, index) =>
    index === 0 ? line.trimStart() : line ? `  ${line.trimStart()}` : "",
  );
...
function updateAtReference(
  ...
  const nextReference =
    candidate.reference.kind === "lagrange" && !candidate.reference.secondary
...
function findNearestSurfaceTarget(
  scene: RenderScene,
...
function findNearestSceneObjectWithThreshold(
  scene: RenderScene,
  selectedObjectId: string,
  pointer: CoordinatePoint,
  threshold: number,
