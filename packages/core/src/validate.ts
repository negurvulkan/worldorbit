...
export function validateDocument(doc: WorldOrbitDocument): void {
  ...
  const objectMap = new Map<string, WorldOrbitObject>();
  ...
  for (const obj of doc.objects) {
    ...
    if (obj.placement.mode === "surface") {
      ...
      if (!target || !SURFACE_TARGET_TYPES.has(target.type)) {
        throw new WorldOrbitError(
          `Surface target "${obj.placement.target}" on "${obj.id}" is not surface-capable`,
        );
      }
    }
...
function validateAnchorReference(
  obj: WorldOrbitObject,
  reference: NamedAnchorReference,
  knownIds: Set<string>,
): void {
  if (!knownIds.has(reference.objectId)) {
    throw new WorldOrbitError(
      `Unknown anchor target "${reference.objectId}" on "${obj.id}"`,
    );
  }
}
