...
import type { RenderHierarchyScene } from "@worldorbit/core";

import type { HierarchySvgRenderOptions } from "./hierarchy-types.js";

export function renderHierarchySceneToSvg(
  scene: RenderHierarchyScene,
  options: HierarchySvgRenderOptions = {},
): string {
...
function renderNode(
  node: RenderHierarchyScene["nodes"][number],
...
function defaultFillForKind(kind: RenderHierarchyScene["nodes"][number]["kind"]): string {
  switch (kind) {
    case "universe":
      return "#95b8ff";
    case "galaxy":
