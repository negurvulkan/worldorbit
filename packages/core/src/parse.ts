...
function parseObjectHeader(tokens: LineToken[], line: number): AstObjectNode {
  ...
  if (!WORLDORBIT_OBJECT_TYPES.has(objectTypeToken.value as WorldOrbitObjectType)) {
    throw new WorldOrbitError(
...
function parseThemeField(tokens: LineToken[], line: number): AstFieldNode {
  if (tokens.length < 2) {
    throw new WorldOrbitError(
      "Invalid theme field line",
      line,
      tokens[0]?.column ?? 1,
    );
  }
