...
export function createInteractiveViewer(
  ...
  if (inputCount !== 1) {
    throw new Error(
      'Interactive viewer requires exactly one of "source", "document", or "scene".',
    );
  }
  ...
  const handleClick = (event: MouseEvent): void => {
    ...
    const objectElement = target.closest<SVGGElement>("[data-object-id]");
    if (!objectElement) {
      applySelection(null);
      return;
    }
  ...
  function rerenderScene(resetView: boolean): void {
    ...
    applySelection(
      state.selectedObjectId &&
        scene.objects.some(
          (object) => object.objectId === state.selectedObjectId && !object.hidden,
        )
        ? state.selectedObjectId
        : null,
      false,
  ...
  function sanitizeState(nextState: ViewerState): ViewerState {
    return {
      ...
      selectedObjectId:
        ...
        scene.objects.some(
          (object) => object.objectId === nextState.selectedObjectId && !object.hidden,
        )
          ? nextState.selectedObjectId
          : null,
    };
  ...
  function applySelection(
    ...
    if (state.selectedObjectId) {
      const previous = container.querySelector<SVGGElement>(
    ...
    if (state.selectedObjectId) {
      const next = container.querySelector<SVGGElement>(
        `[data-object-id="${cssEscape(state.selectedObjectId)}"]`,
      );
