import { loadCosmosSource, type CosmosScope, type LoadedCosmosSource } from "@worldorbit-cosmos/core";
import { createWorldOrbitCosmosViewer, type WorldOrbitCosmosViewer } from "@worldorbit-cosmos/viewer";
...
export function createWorldOrbitCosmosEditor(
  container: HTMLElement,
  options: WorldOrbitCosmosEditorOptions = {},
): WorldOrbitCosmosEditor {
  ...
  const sourceArea = container.querySelector<HTMLTextAreaElement>("[data-cosmos-source]");
  const outline = container.querySelector<HTMLElement>("[data-cosmos-outline]");
  const diagnostics = container.querySelector<HTMLElement>("[data-cosmos-diagnostics]");
  const viewerHost = container.querySelector<HTMLElement>("[data-cosmos-viewer]");
  const scopeSelect = container.querySelector<HTMLSelectElement>("[data-cosmos-scope]");
  const zoomInput = container.querySelector<HTMLInputElement>("[data-cosmos-zoom]");
  const status = container.querySelector<HTMLElement>("[data-cosmos-status]");
  ...
  let loaded: LoadedCosmosSource | null = tryLoad(source);
  let scope: CosmosScope = "system";
  let activeGalaxyId: string | null = loaded?.document.universe.galaxies[0]?.id ?? null;
  let activeSystemId: string | null = loaded?.document.universe.galaxies[0]?.systems[0]?.id ?? null;
  let viewer: WorldOrbitCosmosViewer | null = createWorldOrbitCosmosViewer(viewerRoot, {
    source,
  ...
  scopeField.addEventListener("change", () => {
    scope = scopeField.value as CosmosScope;
  ...
  outlineRoot.addEventListener("click", (event) => {
    ...
    const item = target?.closest?.("[data-cosmos-pick]") as HTMLElement | null;
    if (!item || !loaded) {
      return;
    }
      ...
      ...universe.galaxies.map((galaxy) => {
        const items = [
          ...
          ...galaxy.systems.map((system) =>
            renderOutlineButton("system", system.id, system.title ?? system.id, 2),
          ),
        ];
        return items.join("");
      }),
  ...
  function renderDiagnostics(): void {
    ...
    diagnosticsRoot.innerHTML = list
      .slice(0, 16)
      .map(
        (diagnostic) => `<article class="woc-editor-diagnostic is-${diagnostic.severity}">
...
function tryLoad(source: string): LoadedCosmosSource | null {
  if (!source.trim()) {
    return null;
  }
  try {
