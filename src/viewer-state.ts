...
export function panViewerState(
  state: ViewerState,
  dx: number,
  dy: number,
): ViewerState {
...
export function rotateViewerState(
  state: ViewerState,
  deltaDeg: number,
): ViewerState {
  return {
...
export function fitViewerState(
  ...
  return {
    ...
    scale: nextScale,
    translateX:
      center.x - (center.x + (rotatedCenter.x - center.x) * nextScale),
    translateY:
      center.y - (center.y + (rotatedCenter.y - center.y) * nextScale),
  };
}
...
export function focusViewerState(
  ...
  const target = scene.objects.find(
    (object) => object.objectId === objectId && !object.hidden,
  );
...
export function composeViewerTransform(
  scene: RenderScene,
  state: ViewerState,
): string {
  const center = getSceneCenter(scene);
