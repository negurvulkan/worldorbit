...
export function createWorldOrbitCosmosViewer(container, options = {}) {
    if (!(container instanceof HTMLElement)) {
        throw new Error("WorldOrbit Cosmos viewer requires an HTMLElement container.");
    }
    let source = options.source ?? "";
    let loaded = source ? loadCosmosSource(source) : null;
    ...
    function handleClick(event) {
        ...
        const node = target?.closest?.("[data-cosmos-node-id]");
        if (!node) {
            return;
        }
