...
import type { CosmosRenderScene } from "@worldorbit-cosmos/core";

import type { CosmosSvgRenderOptions } from "./types.js";

export function renderCosmosSceneToSvg(
  scene: CosmosRenderScene,
  options: CosmosSvgRenderOptions = {},
): string {
  ...
  <g data-cosmos-scope="${escapeAttribute(scene.scope)}">
    ${links}
    ${nodes}
  </g>
</svg>`;
...
function renderNode(
  node: CosmosRenderScene["nodes"][number],
  ...
  return `<g class="woc-node${selectedClass}${previewClass}" data-cosmos-node-id="${escapeAttribute(node.id)}" data-cosmos-kind="${escapeAttribute(node.kind)}">
...
function defaultFillForKind(kind: CosmosRenderScene["nodes"][number]["kind"]): string {
  switch (kind) {
    case "universe":
      return "#95b8ff";
    case "galaxy":
