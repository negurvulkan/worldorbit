...
import { createHierarchyViewer, type WorldOrbitHierarchyViewer } from "@worldorbit/viewer";
...
export function createHierarchyEditor(
  container: HTMLElement,
  options: WorldOrbitHierarchyEditorOptions = {},
): WorldOrbitHierarchyEditor {
  ...
  const sourceArea = container.querySelector<HTMLTextAreaElement>("[data-hierarchy-source]");
  const outline = container.querySelector<HTMLElement>("[data-hierarchy-outline]");
  const diagnostics = container.querySelector<HTMLElement>("[data-hierarchy-diagnostics]");
  const viewerHost = container.querySelector<HTMLElement>("[data-hierarchy-viewer]");
  const scopeSelect = container.querySelector<HTMLSelectElement>("[data-hierarchy-scope]");
  const zoomInput = container.querySelector<HTMLInputElement>("[data-hierarchy-zoom]");
  const status = container.querySelector<HTMLElement>("[data-hierarchy-status]");
  ...
  let loaded: LoadedWorldOrbitSource | null = tryLoad(source);
  let scope: WorldOrbitHierarchyScope = "system";
  let savedSource = source;
  let activeGalaxyId: string | null = loaded?.hierarchyDocument?.universe.galaxies[0]?.id ?? null;
  let activeSystemId: string | null = loaded?.hierarchyDocument?.universe.galaxies[0]?.systems[0]?.id ?? null;
  let viewer: WorldOrbitHierarchyViewer | null = createHierarchyViewer(viewerRoot, {
    source,
  ...
  scopeField.addEventListener("change", () => {
    scope = scopeField.value as WorldOrbitHierarchyScope;
  ...
  outlineRoot.addEventListener("click", (event) => {
    ...
    const item = target?.closest?.("[data-hierarchy-pick]") as HTMLElement | null;
    if (!item || !loaded) {
      return;
    }
...
function tryLoad(source: string): LoadedWorldOrbitSource | null {
  if (!source.trim()) {
    return null;
  }
  try {
