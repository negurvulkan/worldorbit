...
export function parseAtlasUnitValue(
  input: string,
  location?: AstSourceLocation,
  fieldKey?: string,
): UnitValue {
...
export function tryParseAtlasUnitValue(input: string): UnitValue | null {
  const match = input.match(UNIT_PATTERN);
  if (!match) {
    return null;
  }
...
export function parseAtlasNumber(
  input: string,
  key: string,
  location?: AstSourceLocation,
): number {
...
export function parseAtlasBoolean(
  input: string,
  key: string,
  location?: AstSourceLocation,
): boolean {
  ...
  if (parsed === undefined) {
    throw WorldOrbitError.fromLocation(
      `Invalid boolean value "${input}" for "${key}"`,
      location,
    );
  }
...
export function parseAtlasFieldBoolean(field: AtlasFieldLike): boolean {
  return parseAtlasBoolean(singleAtlasFieldValue(field), field.key, field.location);
}

export function parseAtlasAtReference(
  target: string,
  location?: AstSourceLocation,
): AtReference {
...
export function validateAtlasImageSource(
  value: string,
  location?: AstSourceLocation,
): void {
  if (!value) {
...
export function normalizeLegacyScalarValue(
  key: string,
  values: string[],
  location: AstSourceLocation,
): NormalizedValue {
  ...
  if (schema.arity === "single" && values.length !== 1) {
    throw WorldOrbitError.fromLocation(
      `Field "${key}" expects exactly one value`,
      location,
    );
  }
...
export function ensureAtlasFieldSupported(
  key: string,
  ...
  if (!schema.objectTypes.includes(objectType)) {
    throw WorldOrbitError.fromLocation(
      `Field "${key}" is not valid on "${objectType}"`,
      location,
    );
  }
}

export function singleAtlasFieldValue(field: Pick<AtlasFieldLike, "key" | "values" | "location">): string {
  return singleAtlasValue(field.values, field.key, field.location);
}

export function singleAtlasValue(
  values: string[],
  key: string,
  location?: AstSourceLocation,
): string {
...
export function isStructureLikeObjectType(objectType: WorldOrbitObjectType): boolean {
  return objectType === "structure" || objectType === "phenomenon";
}

export function cloneNormalizedValue(value: NormalizedValue): NormalizedValue {
...
export function cloneFieldNode(field: AstFieldNode): AstFieldNode {
  return {
    type: "field",
    key: field.key,
    values: [...field.values],
