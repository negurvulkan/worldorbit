...
import type {
  ...
  RenderHierarchySceneLink,
  RenderHierarchySceneNode,
  WorldOrbitGalaxy,
...
export function renderHierarchyDocumentToScene(
  document: WorldOrbitHierarchyDocument,
  options: HierarchySceneRenderOptions = {},
): RenderHierarchyScene {
...
function renderSystemScope(
  document: WorldOrbitHierarchyDocument,
  galaxy: WorldOrbitGalaxy | null,
  system: WorldOrbitHierarchySystem | null,
  width: number,
  height: number,
  padding: number,
  zoom: number,
...
function renderGalaxyScope(
  ...
  const nodes: RenderHierarchySceneNode[] = [];
  const links: RenderHierarchySceneLink[] = [];
  if (!targetGalaxy) {
...
function renderUniverseScope(
  document: WorldOrbitHierarchyDocument,
  width: number,
  height: number,
  padding: number,
  zoom: number,
  activeGalaxy: WorldOrbitGalaxy | null,
  activeSystem: WorldOrbitHierarchySystem | null,
): RenderHierarchyScene {
  const nodes: RenderHierarchySceneNode[] = [];
  const links: RenderHierarchySceneLink[] = [];
  const centerX = width / 2;
...
function emptyContainerScene(
  document: WorldOrbitHierarchyDocument,
  scope: "galaxy" | "universe",
  width: number,
  height: number,
  padding: number,
  zoom: number,
...
function createGalaxyPreviewNodes(
  galaxy: WorldOrbitGalaxy,
  centerX: number,
  centerY: number,
  radius: number,
): RenderHierarchySceneNode[] {
...
function createSystemPreviewNodes(
  system: WorldOrbitHierarchySystem,
  centerX: number,
  centerY: number,
  radius: number,
  parentId: string,
  ...
  return objects.map((object, index) => {
    ...
    return createNode(
      ...
      "object",
      object.id,
      typeof object.properties.class === "string" ? object.properties.class : object.type,
      parentId,
      centerX + Math.cos(angle) * radius * 0.58,
      centerY + Math.sin(angle) * radius * 0.58,
      Math.max(7, radius * 0.1),
      typeof object.properties.color === "string" ? object.properties.color : "#c8d7ff",
      typeof object.properties.image === "string" ? object.properties.image : null,
      false,
      true,
    );
  });
...
function resolveGalaxy(document: WorldOrbitHierarchyDocument, activeGalaxyId?: string | null): WorldOrbitGalaxy | null {
  if (activeGalaxyId) {
...
function createNode(
  id: string,
  kind: RenderHierarchySceneNode["kind"],
  label: string,
...
function createLink(fromId: string, toId: string): RenderHierarchySceneLink {
  return {
