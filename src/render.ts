  ...
  freeObjects.forEach((object, index) => {
    ...
    const y =
      ...
      index *
        Math.max(
          70,
          (height - padding * 2 - 180) / Math.max(1, freeObjects.length),
        );
...
export function renderSourceToSvg(
  source: string,
  options: SvgRenderOptions = {},
): string {
...
function calculateContentBounds(
  ...
  for (const object of objects) {
    ...
    include(
      object.x - object.visualRadius - horizontalPad,
      object.y - object.visualRadius - 12,
    );
    include(
      object.x + object.visualRadius + horizontalPad,
      object.y + object.visualRadius + verticalPad,
    );
  }
