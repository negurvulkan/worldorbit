...
function renderTrajectoryLayer(
  ...
  return scene.trajectories
    ...
    .map((trajectory) => {
      ...
      const waypointMarkup =
        trajectory.showWaypoints && options.showWaypoints
...
function renderTrajectoryWaypoint(
  trajectory: RenderScene["trajectories"][number],
  waypoint: RenderScene["trajectories"][number]["waypoints"][number],
  showLabels: boolean,
): string {
  const labelMarkup =
    showLabels && trajectory.labelMode !== "hidden"
      ? [
          waypoint.label
